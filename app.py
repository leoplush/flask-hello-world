...
def monitor_trade(trade_id, action, entry_price, initial_units, be_pips, tp1_pips, pair):
    ...
    while True:
        ...
        if pips_gained >= be_pips and not be_triggered:
            ...
            be_triggered = True
            print(f"Intento Break Even en Trade {trade_id}. Status: {res.status_code}")
        ...
        if pips_gained >= tp1_pips and not tp1_triggered:
            ...
            tp1_triggered = True
            print(f"Intento TP1 en Trade {trade_id}. Status: {res.status_code}")

        if be_triggered and tp1_triggered:
            print(f"Monitoreo finalizado para Trade {trade_id}.")
            break
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            ...
            if "tradeOpened" in fill_info:
                ...
                trade_id = fill_info["tradesClosed"][0].get("tradeID")
